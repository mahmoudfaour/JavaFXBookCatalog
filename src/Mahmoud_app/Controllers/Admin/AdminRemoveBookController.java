...
public class AdminRemoveBookController implements Initializable{
	...
	public Button remove_btn;
		...
		public void initialize(URL url, ResourceBundle resourceBundle) {
			...
			remove_btn.setOnAction(e->onDeleteBook_button());
		}
...
private void onDeleteBook_button() {
    ...
    try {
        ...
        if (bookName.equals("") || authorName.equals("") || amountStr.equals("")) {
            Model.getInstance().showAlert("Insufficient information", "Please enter all the required book details to delete");
        } else if (amount < 0) {
            Model.getInstance().showAlert("Amount error", "The amount should be positive ");
        } else {
            Model.getInstance().evaluateClientAdminDeleteBook(bookName, authorName, amount);
        }
